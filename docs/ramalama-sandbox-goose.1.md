...
- **NVIDIA GPUs**: vulkan (Linux/macOS) or cuda (WSL2)
...
- On **WSL2**, Vulkan is a poor default, so vendor-specific backends (rocm, cuda, sycl) are
  preferred. It remains available, and `--backend=vulkan` still selects it. This covers both
  Windows, where containers run in the WSL2-backed machine, and ramalama running inside a WSL2
...
ramalama sandbox goose granite

# Force Vulkan backend
ramalama sandbox goose --backend vulkan granite

# Force ROCm backend on AMD GPU
ramalama sandbox goose --backend rocm granite
```
...
[//]: # (BEGIN included file options/goose-image.md)
#### **--goose-image**=*IMAGE*
Goose container image

[//]: # (END   included file options/goose-image.md)
...
```
ramalama sandbox goose --image ghcr.io/ggml-org/llama.cpp:full-vulkan MODEL
```
