...
- **NVIDIA GPUs**: cuda
- **Intel GPUs**: vulkan (Linux/macOS) or intel (Windows)
...
- On **Windows**, vulkan is not supported on WSL2, so vendor-specific backends (rocm, intel) are preferred
...
- **intel**: Use Intel GPU backend (Intel GPUs only)
...
based on GPU type. Use `--backend` to select vulkan, rocm, cuda, or intel backends, which will
...
|  Intel GPU              | quay.io/ramalama/intel-gpu |
