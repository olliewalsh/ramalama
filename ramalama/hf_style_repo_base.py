...
from ramalama.common import available, download_and_verify, exec_cmd, generate_sha256, run_cmd, verify_checksum
...
class HFError(KeyError):
    # Note: KeyError is magic (TODO: it shouldn't be), see cli.py
    pass
...
def fetch_data_from_api_base(api_url, headers=None, extractor_func=None):
    ...
    extractor_func (callable, optional): Optional function to extract the requred data from the response.
...
class HFStyleRepoModel(Model, ABC):
    ...
    def _pull_with_model_store(self, args):
        ...
        try:
            ...
            logger.debug("Failed to pull model", exc_info=True)
